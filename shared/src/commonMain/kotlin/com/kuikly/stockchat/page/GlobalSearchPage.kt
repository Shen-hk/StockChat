...
internal class GlobalSearchPage : BasePager() {
    ...
    /** R4：首帧挂载在起始位，下一帧翻转为终态，驱动全局搜索的阶梯入场。 */
    ...
    override fun pageDidAppear() {
        ...
        if (!reduceMotion) {
            // 初始帧已经登记 easeOut；本次 flip 才会按 R5 消费它。
            setTimeout(0) { entranceVisible = true }
            // 生命周期链路极端丢失时，不能让内容停在透明态。
    ...
    override fun body(): ViewBuilder {
        ...
        return {
            ...
            Scroller {
                ...
                // 搜索框：纯色胶囊
                SearchRevealBlock(0, { page.entranceVisible }, page.reduceMotion) {
                    View {
                        attr {
                ...
                SearchRevealBlock(1, { page.entranceVisible }, page.reduceMotion) {
                ...
                SearchRevealBlock(2, { page.entranceVisible }, page.reduceMotion) {
                    InsightSectionTitle("股票与指数", "名称、代码、全拼与首字母", page.theme)
                }
                // 每支股票是独立动画节点，序号由 vforIndex 提供，避免在普通 builder
                // 中读取 ObservableList（R1），并严格沿用欢迎引导卡的阶梯节奏。
                vforIndex({ page.securities }) { security, index, _ ->
                    SearchRevealBlock(3 + index, { page.entranceVisible }, page.reduceMotion) {
                        View {
                            attr {
                ...
                SearchRevealBlock(3, { page.entranceVisible }, page.reduceMotion) {
...
/** 欢迎页引导卡同款：上滑 28% 自身高度、淡入，并按内容块依次错开。 */
private fun ViewContainer<*, *>.SearchRevealBlock(
    index: Int,
    visible: () -> Boolean,
    ...
    View {
        attr {
            ...
            if (!reduceMotion) {
                ...
                // 每一帧均登记同一动画；visible 翻转时按 R5 消费初始帧的 easeOut。
