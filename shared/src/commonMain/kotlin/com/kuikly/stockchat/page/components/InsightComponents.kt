...
fun ViewContainer<*, *>.InsightSectionTitle(title: String, subtitle: String, theme: StockChatTheme) {
    View {
        ...
        Text { attr { text(subtitle); marginLeft(8f); marginBottom(1f); fontSize(10f); color(theme.textTertiary) } }
    }
}
...
fun ViewContainer<*, *>.ExplanationCard(text: String, stamp: SourceStamp, theme: StockChatTheme) {
    View {
        ...
        Text { attr { text("股问解读"); fontSize(10f); fontWeightSemiBold(); color(theme.brand) } }
        Text { attr { text(text); marginTop(7f); fontSize(13f); lineHeight(20f); color(theme.textPrimary) } }
...
fun ViewContainer<*, *>.SourceStampLine(stamp: SourceStamp, theme: StockChatTheme) {
    View {
        ...
        View {
            ...
            Text { attr { text(stamp.tier.label); fontSize(9f); color(if (stamp.mode.name == "OFFLINE") theme.textTertiary else theme.brand) } }
        }
        Text { attr { text("${stamp.source} · 截至 ${stamp.asOf.ifEmpty { "待更新" }}"); marginLeft(7f); fontSize(9f); color(theme.textTertiary) } }
    }
}
...
fun ViewContainer<*, *>.MetricTile(label: String, value: String, note: String, theme: StockChatTheme, accent: Color = theme.textPrimary) {
    View {
        ...
        Text { attr { text(value); marginTop(5f); fontSize(16f); fontWeightBold(); color(accent) } }
        Text { attr { text(note); marginTop(3f); fontSize(9f); color(theme.textSecondary) } }
    }
}
