...
object LocalAlertProvider : AlertProvider {
    override fun evaluate(rule: AlertRule, quote: Quote): AlertTrigger? {
        if (!rule.enabled || kotlin.math.abs(quote.changePercent) < rule.thresholdPercent) return null
...
class AlertStore(
    ...
    fun upsert(symbol: String, name: String, thresholdPercent: Double = 3.0) {
        val rows = read().filterNot { it.symbol == symbol }.toMutableList()
        rows += AlertRule(symbol, name, thresholdPercent.coerceIn(0.5, 20.0), true, nowMillis())
        write(rows)
    }
