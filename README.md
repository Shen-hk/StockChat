...
- Trie 股票/术语实体识别、会话优先消歧和人工标的选择；单击股票实体进入详情，长按则在顶部灵动岛预览行情，并可直接切换自选或进入详情
- 股票详情页、指标区、图表、归因和资讯区
- 个股资金流、财报、股东户数、龙虎榜、分红事项、公告正文摘要和券商研报，并为结论显示信源与数据时间
- 市场总览、板块热点、涨停池、财报日历和代码/名称/拼音/术语全局搜索
- 应用内异动预警（前台轮询 + 归因说明）、自选分组和异动标记
- 回答前显式复述问题理解并可一键纠正；术语支持一句话、一段话、例子与数据三档解释深度
- Android、iOS 与 H5 分享长图；交易台账和持仓结构分析按路线图范围提供概念卡
- 腾讯行情接口适配，失败时自动回退到设备本地行情缓存或确定性的离线演示数据
- 东方财富公开接口适配，覆盖资金、基本面、披露和市场广度，并使用“在线 → 内存缓存 → 确定性离线数据”降级链
