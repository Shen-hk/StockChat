...
| **R2** | `**/component/**`（含 `page/components/`）不得 import `Repository` / `Store` / `Provider` / `Bridge` / `SharedPreferences` / `PagerScope`，也不得 import `page.*` | 组件只应拿「只读 accessor + Actions」；直接抓数据源会让 UI 与存储/网络耦合，也让组件无法跨 Feature 复用 |
| **R3** | `**/state/**` 不得 import `page.*`、Kuikly View、具体平台实现（`core.module`） | 状态机必须能在没有 UI 的环境下跑单测（项目里已有一批 fake scheduler 测试就是这么做的） |
...
- **R2 的「不得 import `page.*`」是本项目最密集的违反点**（2026-09-12 实测 63 条中的 50 条集中在 `detail/page/component/*`），由 `docs/46` 的 A-3 / A-4 清账。
