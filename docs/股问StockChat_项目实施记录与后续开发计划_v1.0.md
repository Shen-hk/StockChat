...
- 当前主要缺口集中在 Android 真机网络/手势验收、卡片缺字段回归，以及自动化交付。
...
| 行情数据 | Tencent、设备本地缓存与离线演示已由 QuoteRepository 统一；聊天/预览/详情共用 | 部分完成 | 真机断网验收和数据模式回放 |
| 卡片体系 | 8 类模型、协议混排、画廊、Registry 分发、三密度与聊天内渐进披露 | 部分完成 | 缺字段回归和真机体验验收 |
...
- 快照、分时与日/周/月 K 线已写入设备本地缓存，应用重启后可在 TTL 内继续复用；
...
- 已定义 FULL / COMPACT / MINI 三种密度，Gallery 可同时核验三档展示；聊天中的 COMPACT 卡支持展开完整内容，同一时刻仅保留一张展开卡。
...
| 优先级 | 问题 | 影响 | 处理方向 |
|---|---|---|---|
| P0 | 行情设备本地缓存已接入，尚缺真机断网验收 | 三种数据模式尚无设备证据 | 完成在线/缓存/离线回放和截图/录屏留证 |
| P0 | SSE、取消和会话恢复尚未完成真机演练 | 运行时网络行为尚缺设备证据 | 使用真实 Key 验收首字、停止、恢复和错误分类 |
| P0 | H5 由浏览器前端保存 Key 并直连 | 无法安全隐藏密钥，可能受 CORS 限制 | 正式环境使用自有后端代理 |
| P1 | 卡片已按交互形式分流：手风琴、资讯 Sheet、归因下钻、并排对比与嵌套对话；画廊可独立操作验收 | 焦点放大需处理长按与卡内按钮的手势竞争；Sheet 手势仍需真机调校 | 用固定问题集走查展开、Sheet 档位、下钻层级、差值条与分支追问 |
...
当前工作区进度：K 线 Canvas、MA、分时/日周月切换、腾讯原生周期序列、Registry 分发、密度矩阵和聊天内折叠展开已完成；缺字段 UI 回归仍待补。
...
| v1.0 | 2026-08-25 | 未提交工作区 | 行情缓存持久化：快照、分时和日/周/月 K 线写入设备本地存储，重启后可在 TTL 内继续走缓存模式；补充仓库恢复测试 |
| v1.0 | 2026-08-25 | 未提交工作区 | 数据模式可见性增强：统一实时/缓存/离线行情文案，仓库兜底读取可准确返回缓存或离线模式，便于真机断网回放验收 |
| v1.0 | 2026-08-25 | 未提交工作区 | P1 卡片三密度矩阵首版：Gallery 展示所有业务卡片的 FULL / COMPACT / MINI，非行情卡补充 MINI 轻量渲染和 COMPACT 信息裁剪 |
| v1.0 | 2026-08-25 | 未提交工作区 | P1 渐进披露首版：聊天 COMPACT 卡支持展开/收起完整内容，以消息内卡片 ID 维持状态，并保证同一时刻仅展开一张；MINI 长按预览条保持简洁展示 |
| v1.0 | 2026-08-25 | 未提交工作区 | 渐进披露规范适配：CardModel 声明 ExpandMode；行情/术语采用手风琴、资讯采用页面级三档 Sheet、归因因子采用卡内递进下钻；其余增强形式保留后续迭代 |
